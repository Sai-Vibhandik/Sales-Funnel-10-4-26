...
exports.getCreativeStrategy = async (req, res, next) => {
  try {
    ...
    // Note: Stage access is now validated by middleware (checkStageAccess)
    ...
    if (!project.stages.landingPage.isCompleted && !hasLandingPages) {
      return res.status(403).json({
        success: false,
        message: 'Complete the Landing Page stage first to access Creative Strategy'
      });
    }
    ...
    if (!creativeStrategy) {
      ...
      creativeStrategy = await CreativeStrategy.findOneAndUpdate(
        { projectId },
        {
          $setOnInsert: {
