...
export default function ProjectSummary({ projectId, compact = false }) {
  ...
  // Check if user can view strategy data (admin or performance_marketer)
  const canViewStrategy = user?.role === 'admin' || user?.role === 'performance_marketer';
  ...
  const fetchSummary = async () => {
    try {
      ...
      // Only fetch strategy data if user has permission (admin or performance_marketer)
      // AND the stages are accessible
      if (!canViewStrategy) {
        // Team members don't have access to strategy APIs
        // They can only see basic project info
        return;
      }

      // Only fetch strategy data for stages that are accessible
      // Stages must be completed in order: onboarding -> marketResearch -> offerEngineering -> trafficStrategy -> landingPage -> creativeStrategy
      const stages = projectData.stages || {};
      const canFetchMarketResearch = stages.onboarding?.isCompleted;
      const canFetchOffer = canFetchMarketResearch && stages.marketResearch?.isCompleted;
      const canFetchTraffic = canFetchOffer && stages.offerEngineering?.isCompleted;
      const canFetchLandingPage = canFetchTraffic && stages.trafficStrategy?.isCompleted;
      const canFetchCreative = canFetchLandingPage && stages.landingPage?.isCompleted;

      // Fetch market research only if onboarding is complete
      const mrPromise = canFetchMarketResearch
        ? marketResearchService.get(projectId).catch(() => ({ data: null }))
        : Promise.resolve({ data: null });

      // Fetch other strategies only if previous stages are completed
      const offerPromise = canFetchOffer
        ? offerService.get(projectId).catch(() => ({ data: null }))
        : Promise.resolve({ data: null });

      const trafficPromise = canFetchTraffic
        ? trafficStrategyService.get(projectId).catch(() => ({ data: null }))
        : Promise.resolve({ data: null });

      const creativePromise = canFetchCreative
        ? creativeService.get(projectId).catch(() => ({ data: null }))
        : Promise.resolve({ data: null });

      const [mrRes, offerRes, trafficRes, creativeRes] = await Promise.all([
        mrPromise,
        offerPromise,
        trafficPromise,
        creativePromise
      ...
      // Only show if landing page stage is accessible
      if (canFetchLandingPage) {
        setLandingPage(projectData.landingPages?.[0] || null);
      } else {
        setLandingPage(null);
      }
    } catch (error) {
